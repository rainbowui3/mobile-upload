...
export default class Data extends Component {
    ...
    renderPagination() {
        if (this.parseBool(this.props.pageable)) {
            return (
                    <Pagination renderDropDownList={this.renderDropDownList()} pageCount={this.pageCount}
                                currentPageIndex={this.currentPageIndex} onToPage={this.onToPage.bind(this)}/>
            );
        }
    }
    ...
    renderBeforeData() {
        return (
            <div className="row">
                {this.parseBool(this.props.searchable) ? this.renderSearch() : null}
                {this.props.functions != null ? this.renderFunction() : null}
        ...
        if (this.parseBool(this.props.pageable)) {
            ...
            return (
                <div style={{display:'inline',paddingLeft:'15px'}}>
                    {UITools.I18NUtil.default.formatObject(i18n.DropDownInfo, <select style={{width: "50px"}}
                                                                      value={this.state.pageSize}
                                                                      onChange={this.onDropDownChange.bind(this)}>{dropDownArray}</select>)}
                </div>
            );
        }
    }
    ...
    renderFunction() {
        ...
        return (
            <div
                className={this.parseBool(this.props.searchable)?"col-sm-6 col-md-6 col-lg-6":"col-sm-12 col-md-12 col-lg-12"}
    ...
    renderSearch() {
        if (this.parseBool(this.props.searchable)) {
            return (
                <div className="col-sm-6 col-md-6 col-lg-6" style={{textAlign: "left",paddingTop:'5px'}}>
        ...
        if (this.parseBool(this.props.searchable) && this.state.searchValue != undefined) {
            ...
            $.each(value, function (index, element) {
                $.each(columnValue, function (index2, element2) {
                    ...
                    if (val != null && val != undefined && String(val).toUpperCase().indexOf(_self.state.searchValue.toUpperCase()) != -1) {
    ...
    getDisabledStyleClass() {
        if (UITools.ELUtil.parseBoolValue(this.props.disabled)) {
            return "disabled";
        }
        return null;
    }
    ...
    onToPage(currentPageIndex) {
        ...
        UITools.DataContext.put(this.componentId + "_currentPageIndex", currentPageIndex);
    }
